    ...
    class LocalStorageAdapter{
      ...
      async exportBackup(){const backup=JSON.parse(JSON.stringify({app:"月之暗面",version:2,exportedAt:now(),nextSeq:data.nextSeq,tags:data.tags||[],messageKinds:data.messageKinds||DEFAULT_KINDS,polls:data.polls||[],handoffNotes:data.handoffNotes||[],frontingLogs:data.frontingLogs||[],tasks:data.tasks||[],careLogs:data.careLogs||[],careChecklist:data.careChecklist||[],ledgerRecords:normalizeLedgerRecordsForBackup(getRuntimeLedgerRecordsForBackup()),systemProfile:data.systemProfile||blankSystemProfile(),systemProfileVisibility:normalizeSystemProfileVisibilityRecord(data.systemProfileVisibility),memberRelations:data.memberRelations||[],externalSystemCards:data.externalSystemCards||[],rooms:data.rooms,members:data.members,messages:data.messages})); if(typeof hydrateImagesForJsonExport==="function")await hydrateImagesForJsonExport(backup); return backup;}
    ...
    async function save(){let pendingResolve=null; if(debouncedSaveTimer){clearTimeout(debouncedSaveTimer); debouncedSaveTimer=null; pendingResolve=debouncedSaveResolve; debouncedSavePromise=null; debouncedSaveResolve=null;} try{const ok=await storage.saveAppData(data); if(pendingResolve)pendingResolve(ok); return ok;}catch(err){const isQuota=err?.name==="QuotaExceededError"||/quota|exceed|full/i.test(String(err?.message||"")); if(isQuota){alert("本地存储已满。\n\n这次保存没有写入。当前屏幕上的内容可能还没保存。请：\n1. 先复制屏幕上尚未保存的内容\n2. 去 设置 → 数据 → 导出 JSON 备份\n3. 妥善保存导出的文件\n4. 删除一些旧图片、对话背景或聊天记录后再尝试。");}else{alert("保存失败："+(err?.message||"未知错误")+"\n\n这次保存没有写入。建议先复制当前内容再尝试。");} if(pendingResolve)pendingResolve(false); return false;}}
    function debouncedSave(label="保存",delayMs=DEBOUNCED_SAVE_DELAY_MS){if(debouncedSaveTimer)clearTimeout(debouncedSaveTimer); if(!debouncedSavePromise)debouncedSavePromise=new Promise(resolve=>{debouncedSaveResolve=resolve;}); debouncedSaveTimer=setTimeout(async()=>{debouncedSaveTimer=null; const resolve=debouncedSaveResolve; debouncedSavePromise=null; debouncedSaveResolve=null; const ok=await save(); if(resolve)resolve(ok);},Math.max(0,Number(delayMs)||0)); return debouncedSavePromise.catch(err=>{console.error(`${label}失败`,err); return false;});}
