    ...
    class LocalStorageAdapter{
      ...
      async exportBackup(){return {app:"月之暗面",version:2,exportedAt:now(),nextSeq:data.nextSeq,tags:data.tags||[],messageKinds:data.messageKinds||DEFAULT_KINDS,polls:data.polls||[],handoffNotes:data.handoffNotes||[],frontingLogs:data.frontingLogs||[],tasks:data.tasks||[],ledgerRecords:normalizeLedgerRecordsForBackup(getRuntimeLedgerRecordsForBackup()),systemProfile:data.systemProfile||blankSystemProfile(),systemProfileVisibility:normalizeSystemProfileVisibilityRecord(data.systemProfileVisibility),memberRelations:data.memberRelations||[],externalSystemCards:data.externalSystemCards||[],rooms:data.rooms,members:data.members,messages:data.messages};}
      async importBackup(jsonData){const parsed=typeof jsonData==="string"?JSON.parse(jsonData):jsonData; if(!Array.isArray(parsed.rooms)||!Array.isArray(parsed.members)||!Array.isArray(parsed.messages))throw new Error("invalid_backup"); return migrate({nextSeq:parsed.nextSeq,tags:parsed.tags||[],messageKinds:parsed.messageKinds||DEFAULT_KINDS,polls:parsed.polls||[],handoffNotes:parsed.handoffNotes||[],frontingLogs:parsed.frontingLogs||[],tasks:parsed.tasks||[],systemProfile:parsed.systemProfile,systemProfileVisibility:parsed.systemProfileVisibility,memberRelations:parsed.memberRelations||[],externalSystemCards:parsed.externalSystemCards||[],rooms:parsed.rooms,members:parsed.members,messages:parsed.messages});}
    }
