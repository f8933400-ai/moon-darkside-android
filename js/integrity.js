    ...
    function nextMessageSeq(){const fallback=Math.max(0,...(data.messages||[]).map(m=>Number(m.seq)||0))+1; const seq=Math.max(Number(data.nextSeq)||0,fallback); data.nextSeq=seq+1; return seq;}
