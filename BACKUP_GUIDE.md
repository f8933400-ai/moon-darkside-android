...
因此，PWA 不替代备份流程。P5-04 已将静态壳缓存版本更新为 `moon-app-shell-p5-04-v0.4.0`，并补齐本地图标；更新 app 文件后，如果看到旧界面，可以刷新页面，或清理此站点缓存 / Service Worker 后重新打开。直接用 `file://` 打开时，Service Worker 不可用；需要 PWA 缓存能力时，请使用本机 HTTP 或可信的 HTTPS 本地 / 私有静态服务。
