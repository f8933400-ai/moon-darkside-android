...
7. 如果导入前存在旧式 inline 图片字段，导入成功后会清理图片迁移完成标记，让下次启动能重新检查是否还有旧式图片需要迁移。
8. 如果备份里有已经到期的进行中投票，导入保存前会立即结算，不需要等待定时器。
9. 全部成功后才覆盖当前运行时 data 并保存。
...
这只是序号重算，不改变 `messageIntegrity` 输入字段集合。P6-01 起新生成的校验使用真实 U+001F 分隔符；旧版字面 `"\\u001f"` 分隔符生成的消息仍会兼容验证。
